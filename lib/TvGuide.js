...
TvGuide.prototype.getShow = function(showName) {
  ...
  .catch(() => {
    try {
      if (showName.match(/\d/g).length > 0) {
        let newShowToLookup = showName.replace(/\d/g, function(something) {
...
TvGuide.errorHanding = (error, showQuery) => {
  if (Object.keys(TvGuide.throwables).indexOf(error.name) === -1) {
    // if we dont have a handler for this error, return a generic one.
    console.log(error);
    return new GenericShowLookupError.message(showQuery);
  }
